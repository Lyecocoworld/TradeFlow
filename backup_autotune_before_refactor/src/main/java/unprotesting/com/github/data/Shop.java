...
public class Shop implements Serializable {
    ...
    public Shop(String name, ResultSet rs, Gson gson) throws SQLException {
        ...
        this.setting = new CollectFirst(collectFirstSettingString != null ? collectFirstSettingString : "none");
    }
    ...
    public boolean isUnlocked(UUID player) {
        Format.getLog().info(String.format("[DEBUG] Shop %s isUnlocked for player %s. CollectFirstSetting: %s", name, player, setting.getSetting()));

        if (!Database.get().areMapsReady()) {
            Format.getLog().info(String.format("[DEBUG] Shop %s isUnlocked for player %s: Database not ready, returning false.", name, player));
            return false; // Lock if database is not ready
        }

        if (setting.getSetting() == CollectFirstSetting.NONE) {
            Format.getLog().info(String.format("[DEBUG] Shop %s isUnlocked for player %s: NONE setting, returning true.", name, player));
            return true;
        }
        if (setting.getSetting() == CollectFirstSetting.PLAYER) {
            boolean collected = Database.get().hasPlayerCollected(player, name);
            Format.getLog().info(String.format("[DEBUG] Shop %s isUnlocked for player %s: PLAYER setting, hasPlayerCollected: %s", name, player, collected));
            return collected;
        }
        // SERVER mode logic (needs implementation)
        Format.getLog().info(String.format("[DEBUG] Shop %s isUnlocked for player %s: Unknown/SERVER setting, returning true for now.", name, player));
        return true;
    }
    ...
    public static Shop fromConfig(String name, ConfigurationSection config, String sectionName, boolean enchantment) {
        ...
        double startPrice = config.getDouble("price");
        String collectFirstSettingString = config.getString("collect-first", Config.get().getDefaultCollectFirst());
        Shop shop = Shop.builder()
                ...
                .size(1)
                .setting(new CollectFirst(collectFirstSettingString))
