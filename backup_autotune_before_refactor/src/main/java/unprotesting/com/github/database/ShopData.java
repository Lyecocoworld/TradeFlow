...
public class ShopData {
    ...
    public ShopData(AutoTune plugin, MySQLConnector connector) {
        this.plugin = plugin;
    ...
    public void saveShop(Shop shop, String id) {
        String query = "INSERT INTO shops (id, price, enchantment, locked, volatility, section, max_buys, max_sells, collect_first_setting, " +
                "buys_history, sells_history, prices_history, autosell, recent_buys, recent_sells) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) " +
                "ON DUPLICATE KEY UPDATE price=?, enchantment=?, locked=?, volatility=?, section=?, max_buys=?, max_sells=?, collect_first_setting=?, " +
                "buys_history=?, sells_history=?, prices_history=?, autosell=?, recent_buys=?, recent_sells=?;";
        ...
        try (Connection conn = connector.getConnection();
            ...
            ps.setString(9, shop.getSetting().getSetting().toString()); // New parameter
            ps.setString(10, gson.toJson(shop.getBuys()));
            ps.setString(11, gson.toJson(shop.getSells()));
            ps.setString(12, gson.toJson(shop.getPrices()));
            ps.setString(13, gson.toJson(shop.getAutosell()));
            ps.setString(14, gson.toJson(shop.getRecentBuys()));
            ps.setString(15, gson.toJson(shop.getRecentSells()));
            ...
            ps.setString(23, shop.getSetting().getSetting().toString()); // New parameter
            ps.setString(24, gson.toJson(shop.getBuys()));
            ps.setString(25, gson.toJson(shop.getSells()));
            ps.setString(26, gson.toJson(shop.getPrices()));
            ps.setString(27, gson.toJson(shop.getAutosell()));
            ps.setString(28, gson.toJson(shop.getRecentBuys()));
            ps.setString(29, gson.toJson(shop.getRecentSells()));
