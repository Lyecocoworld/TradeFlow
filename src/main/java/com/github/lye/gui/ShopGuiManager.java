...
public class ShopGuiManager {
    ...
    public ShopGuiManager(@NotNull TradeFlow plugin, AccessResolver accessResolver, ConfigResolver configResolver, Database database, Config config, com.yourplugin.pricing.service.PriceService priceService, com.yourplugin.pricing.gui.FamilyRegistry familyRegistry) {
        this.plugin = plugin;
    ...
    protected List<Component> getLoreForEnchantment(@NotNull Player player, @NotNull String name, int level,
        ...
        try {
            Shop shop = ShopUtil.getShop(this.database, name, true);
            double base = shop.getPrice();
            double price = base * level;

            // Change formatting same as getLore()
            String change = Format.percent(shop.getChange());
            if (shop.getChange() > 0) {
                change = "<green>" + change + "</green>";
            } else if (shop.getChange() < 0) {
            ...
            TagResolver resolver = TagResolver.resolver(
                    ...
                    Placeholder.parsed("total-price", Format.currency((double) amount * price)),
                    Placeholder.parsed("buys-left", Format.number(ShopUtil.getBuysLeft(this.database, player, name))),
                    Placeholder.parsed("max-buys", Format.number(shop.getMaxBuys())),
                    // Sell context (populate even if you don't sell books, for template compatibility)
                    Placeholder.parsed("sell-price", Format.currency(shop.getSellPrice() * level)),
                    Placeholder.parsed("total-sell-price", Format.currency((double) amount * shop.getSellPrice() * level)),
                    Placeholder.parsed("sells-left", Format.number(ShopUtil.getSellsLeft(this.database, player, name))),
                    Placeholder.parsed("max-sells", Format.number(shop.getMaxSells())),
                    // Common
                    Placeholder.parsed("amount", Format.number(amount)),
