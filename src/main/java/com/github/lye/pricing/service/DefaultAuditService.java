...
public class DefaultAuditService implements AuditService {
    ...
    public void logGraphAndFamilyStats(int totalItems, int cyclesDetected, int familiesCount, int variantsCount) {
        logger.info(String.format("[Pricing] Built dependency graph: %d items, %d reversible cycles.", totalItems, cyclesDetected));
        logger.info(String.format("[Pricing] Families: %d roots, %d variants.", familiesCount, variantsCount));
    }

    @Override
    public void logDatabaseStatus(boolean isMySQLActive) {
        logger.info(String.format("[Pricing] MySQL %s, MapDB in standby", isMySQLActive ? "active" : "inactive"));
    }

    @Override
    public void logGuiInitialization(int pages, int families) {
        logger.info(String.format("[Pricing] GUI initialized: %d pages, %d families", pages, families));
    }

    @Override
    public void logWarning(String message) {
        logger.warning("[Pricing] " + message);
    }

    @Override
    public void logInfo(String message) {
        logger.info("[Pricing] " + message);
    }
